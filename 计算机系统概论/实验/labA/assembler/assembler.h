...
inline std::string& LeftTrim(std::string& s, const char* t = " \t\n\r\f\v") {
    // finish to be done
    const auto StrBegin = s.find_first_not_of(t);
    if (StrBegin == std::string::npos) s = "";
    else s.erase(0, StrBegin);
    return s;
}//some problem
...
inline std::string& RightTrim(std::string& s, const char* t = " \t\n\r\f\v") {
    // finish to be done
    const auto StrEnd = s.find_last_not_of(t);
    if (StrEnd == std::string::npos) s = "";
    else s.erase(StrEnd + 1);
    return s;
}//some problem 
